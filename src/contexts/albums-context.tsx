...
interface AlbumsContextType {
  ...
  addPhotoToAlbum: (photoId: string, albumId: string, thumbUrl: string) => Promise<void>
...
export function AlbumsProvider({ children }: { children: ReactNode }) {
  ...
  const loadAlbums = useCallback(async () => {
    ...
    try {
      // Try cache first
      const cached = await localforage.getItem<Album[]>(`${ALBUMS_CACHE_KEY}_${user.uid}`)
      ...
      await localforage.setItem(`${ALBUMS_CACHE_KEY}_${user.uid}`, loadedAlbums)
    } catch (error) {
  ...
  const addPhotoToAlbum = useCallback(
    async (photoId: string, albumId: string, thumbUrl: string) => {
      ...
      if (album && !album.coverPhotoId) {
        ...
        setAlbums((prev) => prev.map((a) => (a.id === albumId ? { ...a, photoCount: a.photoCount + 1 } : a)))
      }
    },
    [albums],
  )
          ...
          prev.map((a) =>
            a.id === albumId
              ? {
                  ...a,
                  photoCount: newCount,
                  coverPhotoId: album.coverPhotoId === photoId ? null : a.coverPhotoId,
                  coverPhotoUrl: album.coverPhotoId === photoId ? null : a.coverPhotoUrl,
                }
              : a,
          ),
        )
  ...
  const getAlbumPhotos = useCallback(async (albumId: string): Promise<Photo[]> => {
    const photosRef = collection(db, "photos")
    const q = query(photosRef, where("albumIds", "array-contains", albumId), orderBy("createdAt", "desc"))
    const snapshot = await getDocs(q)
